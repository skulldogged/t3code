...
for (const location of ["root", "nested", "worktree", "without-divergence"] as const) {
  it.effect(
      ...
      Effect.gen(function* () {
        ...
        for (let poll = 0; poll < 3; poll++) {
          const result = yield* driver
            .statusDetailsLocal(
              statusCwd,
              location === "without-divergence" ? { includeDivergence: false } : undefined,
            )
            .pipe(Effect.result);
        ...
        const status = yield* driver.statusDetailsLocal(
          statusCwd,
          location === "without-divergence" ? { includeDivergence: false } : undefined,
        );
