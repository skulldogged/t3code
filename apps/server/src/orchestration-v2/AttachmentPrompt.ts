...
export function providerMessageTextWithAttachmentPaths(input: {
  ...
  let inputTextWithAttachmentContext = input.text;
  const appendAttachmentContext = (context: string | undefined) => {
    if (!context) return;
    const candidate = [inputTextWithAttachmentContext, context].filter(Boolean).join("\n\n");
    if (candidate.length <= PROVIDER_SEND_TURN_MAX_INPUT_CHARS)
      inputTextWithAttachmentContext = candidate;
  };
  appendAttachmentContext(paths.join("\n"));
