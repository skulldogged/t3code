...
function useResolvedThreadSelection(params: ThreadSelectionRouteParams | undefined) {
  ...
  const selectedThreadDetailShell = useMemo(
    () =>
      selectedThreadRef !== null && selectedThreadDetail !== null
        ? threadDetailToShell(selectedThreadRef.environmentId, selectedThreadDetail)
        : null,
    [selectedThreadDetail, selectedThreadRef],
  );
  const selectedThread = useMemo(
    () =>
      selectedThreadShell ??
      selectedThreadDetailShell ??
      (pendingCreation !== null ? pendingThreadCreationShell(pendingCreation.message) : null),
    [pendingCreation, selectedThreadDetailShell, selectedThreadShell],
  );
    ...
    detail:
      selectedThreadDetail === null
        ...
        : {
            ...
            latestRun: selectedThreadDetailShell?.latestRun ?? null,
            runtime: selectedThreadDetailShell?.runtime ?? null,
          },
  });
